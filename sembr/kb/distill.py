...
- 每条线索给:`title`(简短线索名,不带 markdown)、`section`(主题分节,如 海峡通行与管控/军事冲突/石油市场/外交与国际反应/停火与和平协议 等)、`first_seen`(首见 ISO 日期 YYYY-MM-DD)、`current_state`(最新当前状态一句话,含关键数字)、`timeline`(按日期推进的进展条目数组,每条 {date: YYYY-MM-DD, entry: 一句话进展})。
- timeline 体现该线索【怎么一步步演进】,不是只给最新;每个日期最多一条(把当天该线索的要点合并成一句)。

只输出 JSON,形如 {"threads":[{"title":"...","section":"...","first_seen":"2026-06-01","current_state":"...","timeline":[{"date":"2026-06-01","entry":"..."}]}]}。不要任何解释。"""
...
def render_threads(threads: list[_DistillThread], now_date: str) -> str:
    ...
    for t in threads:
        base = _merge.slugify(t.title) or _merge.stable_fallback_key(t.title)
        key = base
        n = 2
        while key in used:
            key = f"{base}-{n}"
