...
async def test_override_replaces_and_refreshes_created_at(mem_conn):
    ...
    assert await count_for(mem_conn, intent.id, _VER) == 1  # still exactly one row
...
async def test_count_for_isolates_intent_and_version(mem_conn):
    ...
    assert await count_for(mem_conn, a.id, _VER) == 2  # two articles, this version
    assert await count_for(mem_conn, a.id, "v2") == 1  # different version isolated
    assert await count_for(mem_conn, b.id, _VER) == 1  # different intent isolated
...
async def test_delete_intent_cascades_cache(mem_conn):
    ...
    assert await count_for(mem_conn, intent.id, _VER) == 1
    await delete_intent(mem_conn, intent.id)
    assert await count_for(mem_conn, intent.id, _VER) == 0  # FK ON DELETE CASCADE
