...
These three cases lock the LEGACY pure-delete path
(``qdrant_archive_enabled=False``): they predate the archive migration and
their mocks only model scroll+delete. The archive-enabled pipeline has its
own suite in ``test_qdrant_archive_ttl.py``.
"""
...
_LEGACY_SETTINGS = Settings(qdrant_archive_enabled=False)
...
async def _close_conn(conn: aiosqlite.Connection) -> None:
    # try/finally caller convention: a failed assert must still close the
    # connection or the aiosqlite thread leaks and hangs pytest on CI.
    await conn.close()
...
async def test_qdrant_ttl_cascade_delete():
    ...
    try:
        ...
        await _run_qdrant_ttl(qdrant, _LEGACY_SETTINGS)

        qdrant.client.delete.assert_awaited()  # at least one Qdrant delete batch
        # Legacy path must never touch the archive pipeline.
