...
GENERAL_CN = [
    (
        "新华网",
        "https://xinhuarss.xinhua.org/zh_news_2016010601/rss.xml",
        False,
    ),  # old URL 404, try new
...
def _probe_one(name: str, url: str, timeout: int, min_full_len: int) -> ProbeResult:
    ...
    if not result.parse_ok and result.entry_count == 0:
        result.verdict = "ERROR"
        result.notes.append(f"parse failed bozo_exc={feed.get('bozo_exception', '')}")
        return result
...
def _print_cadence(all_results: list[list[ProbeResult]], section_names: list[str]) -> None:
    ...
    for results in all_results:
        for r in results:
            ...
            warn = (
                " <-- increase!"
                if (r.safe_poll_minutes is not None and r.safe_poll_minutes < 30)
                else ""
            )
...
def _print_summary(all_results: list[list[ProbeResult]]) -> None:
    ...
    print(f"\n{'=' * 80}")
    print(
        f"  OVERALL: {len(flat)} feeds -- FULL={len(full)}  STUB/SUMMARY={len(stub)}  ERROR={len(err)}"
    )
    print(f"{'=' * 80}")
