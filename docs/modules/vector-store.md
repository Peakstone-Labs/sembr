...
It runs every 30 minutes with a 2-minute startup offset and a 10-minute per-round budget — deliberately not on the 24-hour maintenance cadence, because until the queue drains every derived-field search carries an under-recall warning, and a round cut short by the budget has to resume in minutes rather than the next day. A batch that writes without shrinking the queue aborts the round; three consecutive rounds stalling on the same batch quarantine those ids so one unwritable point cannot burn every future round. Past a ceiling on the quarantine set the job stops running rounds altogether — the set is replayed as a `must_not has_id` on every query, so an unbounded one would turn the repair job into a load generator. That give-up state is process-local and has no reset path: the search-side warning stays on (correctly, since those points really are missing their fields) but freezes at the last published count, so a deployment that fixes the root cause needs a restart before the search side notices. `GET /api/dashboard/maintenance/qdrant_stats` reports the remaining depth as `derived_backfill_pending` and the given-up count as `derived_backfill_quarantined`, both recomputed on request rather than read back from the job's own bookkeeping — so the operator view is never frozen.
