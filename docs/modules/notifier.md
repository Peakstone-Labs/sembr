...
- **Synchronous SMTP via `asyncio.to_thread`**: a slow or hung MTA holds a thread for the duration of the SMTP exchange. At MVP fan-out (a handful of intents firing per minute) this is fine. A high-fan-out deployment should swap `smtplib` for an async SMTP library or a queue-backed sender
