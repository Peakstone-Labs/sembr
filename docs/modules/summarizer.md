...
- **Templates are read on every call**: no in-process caching. Disk I/O cost is negligible at 1.0 scale (a few intents firing per minute) but a future high-fan-out deployment should add a cache or move templates to a database table.
