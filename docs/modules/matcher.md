...
- **Event scoring is pure Python**: `_dot` walks 1024 floats per (intent, article) pair without numpy. At MVP scale (≤50 event-mode intents, batches up to ~32 articles) the cost is negligible, but a backend swap that increases dim, or scaling event-mode intents into the hundreds, will want a vectorized implementation.
